...
export function OperationDetailEditModal({ operationId, operationDetail, open, setOpen }) {
    const theme = useTheme();
    const xsMatch = useMediaQuery(theme.breakpoints.down("sm"));
    ...
    return (
        <Dialog open={open} maxWidth={"md"} width={"md"} fullWidth scroll={"paper"} fullScreen={xsMatch}>
            <Title title={"Editar detalle de producto"} handleClose={handleClose} />
            <DialogContent>
                <OperationDetailForm operationId={operationId} operationDetail={operationDetail} handleClose={handleClose} />
            </DialogContent>
        </Dialog>
    );
}
